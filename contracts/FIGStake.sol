...
contract FIGStake is Context, ReentrancyGuard {
    ...
    function unStake(uint stakeId) external nonReentrant returns (uint) { 
        ...
        uint withdrawn = data[0];
        if (data[1] > 0) {
            _stat.totalRemain += data[1];
        }
        _token.transfer(staker, stake.amount);
        payable(staker).transfer(withdrawn);
        _stat.totalWithdrawn += withdrawn;
        emit Withdrawn(staker, stakeId, withdrawn);
        ...
        // notify event
        emit StakeDropped(staker, stakeId, stake.amount, uint(stake.stakeType));
    ...
    function withdraw() external nonReentrant returns (uint) {
        ...
        for (uint i = 0; i < stakes.length; i++) {
            ...
            emit Withdrawn(staker, stake.id, reward);
        }
