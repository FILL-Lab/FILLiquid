...
contract FILLiquid is Context, IFILLiquid {
    ...
    IValidation private _validation;
    ...
    constructor(address filTrustAddr, address validationAddr, address calculationAddr, address filecoinAPIAddr, address fitStakeAddr, address governanceAddr, address payable foundationAddr) {
        _tokenFILTrust = IFILTrust(filTrustAddr);
        _validation = IValidation(validationAddr);
        _calculation = ICalculation(calculationAddr);
        _filecoinAPI = IFilecoinAPI(filecoinAPIAddr);
        _fitStake = IFITStake(fitStakeAddr);
        _owner = _msgSender();
    ...
    function borrow(uint64 minerId, uint amount, uint expectInterestRate) external isBindMiner(minerId) haveCollateralizing(minerId) returns (uint, uint) {
        ...
        if(fees[0] > 0) {
            (bool success, ) = address(_filecoinAPI).delegatecall(
                abi.encodeCall(IFilecoinAPI.send, (minerId, fees[0]))
            );
    ...
    function setAdministrativeFactors(
        ...
        _validation = IValidation(new_validation);
        _calculation = ICalculation(new_calculation);
        _filecoinAPI = IFilecoinAPI(new_filecoinAPI);
    ...
    function changeBeneficiary(
        ...
        (bool success, ) = address(_filecoinAPI).delegatecall(
            abi.encodeCall(IFilecoinAPI.changeBeneficiary, (minerId, beneficiary, quota, expiration))
        );
    ...
    function withdrawBalance(uint64 minerId, uint withdrawnAmount) private{
        if (withdrawnAmount > 0) {
            (bool success, bytes memory data) = address(_filecoinAPI).delegatecall(
                abi.encodeCall(IFilecoinAPI.withdrawBalance, (minerId, withdrawnAmount))
            );
