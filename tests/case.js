...
// console.log("ethersRaw: ", ethersRaw)
// process.exit()
...
describe("Liquid", function () {
  async function deployLiquid() {
    ...
    const FILStake = await hre.ethers.getContractFactory("FITStake");
    ...
    await governance.setContractAddrs(filLiquid.address, filStake.address, filGovernance.address)
    await filStake.setContractAddrs(filLiquid.address, governance.address, filTrust.address, calculation.address, filGovernance.address)
  ...
  describe("Test flow", function () {
    it("Test all", async function () {
      ...
      for (stepIndex in stepList) {
        ...
        console.log(`stepIndex: ${stepIndex} step.contractName: ${step.contractName} step.functionName: ${step.functionName} step.increaseBlockNumber: ${step.increaseBlockNumber} step.value ${step.value}`)

        if(step.functionName === "directPayback") {
          // let result = await contracts.filLiquid.maxBorrowAllowed(step.params[0])
          
          // let result = await contracts.filLiquid.getAvailableBalance(step.params[0])
          // let result = await contracts.filLiquid.minerBorrows(step.params[0])
          // console.log("minerBorrows: ", result)
        }
      ...
      for (stepIndex in finalStateCheckList) {
        ...
        console.log(`checkStepIndex: ${stepIndex} step.contractName: ${step.contractName} step.functionName: ${step.functionName} step.increaseBlockNumber: ${step.increaseBlockNumber} step.value ${step.value} step.params: ${step.params} step.results: ${step.results}`)
